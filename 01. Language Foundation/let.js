...
// * 2. 全局声明 —— let 在全局作用域中声明的变量不会成为 window 对象的属性。
/* 不过 let 声明仍然是在全局作用域中发生的，相应变量会在页面的声明周期内存续。为了避免 SyntaxError 必须确保页面不会
  重复声明同一个变量。 */
var name = "Matt";
let age = 26;
...
// * 5. 作用域与上下文 —— 使用 var 声明变量时，变量会被自动添加到最接近的上下文
/* 在函数中最接近的上下文就是函数的局部上下文。如果变量未经声明就被初始化了，那么它就会被自动添加到全局上下文。 */
